...
private case class CacheEntry[V](value: V, expiresAt: ZonedDateTime) {
...
trait CacheWithFallbackToStaleData[K, V] {
  ...
  private[this] val cache =
    new java.util.concurrent.ConcurrentHashMap[K, CacheEntry[V]]()
  private val logger: Logger = LoggerFactory.getLogger(getClass)
  private[this] val DefaultDurationSecondsForNone = 2L

  /**
   * If the result of the operation is either None, defines
   * how long we cache this value for. A common use case can be
   * is caching the lookup of an item - this allows us
  ...
  def flush(key: K): Unit = {
    cache.computeIfPresent(key, (_: K, entry: CacheEntry[V]) => {
      entry.copy(
        expiresAt =
          ZonedDateTime.now.minus(1, ChronoField.MILLI_OF_DAY.getBaseUnit))
    })
    ()
  }
    ...
    val finalEntry = Option(cache.get(key)) match {
      ...
      case None =>
        cache.computeIfAbsent(key,
          (k: K) => doGetEntry(k)(failureFromEmpty(k, _)))
    }
    finalEntry.value
  }
  ...
  private[this] def failureFromEmpty(key: K, ex: Throwable): CacheEntry[V] = {
    val msg =
      s"Error for Cache[${this.getClass.getName}] key[$key]: ${ex.getMessage}"
    logger.error(msg, ex)
    sys.error(msg)
  }

  private[this] def failureFromRefresh(key: K,
                                       currentEntry: CacheEntry[V],
                                       ex: Throwable): CacheEntry[V] = {
    logger.warn(
      s"Cache[${this.getClass.getName}] key[$key]: Falling back to stale data " +
        s"as refresh failed with: ${ex.getMessage}",
      ex)
    currentEntry
  }
